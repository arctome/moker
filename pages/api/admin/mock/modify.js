...
export default async function ApiAdminMockRecordModify(event) {
    ...
    let modRes = await MockRecord.del(name, data.mock_id).catch(e => {
    ...
    if (!modRes) return new Response(JSON.stringify({
        ok: 0,
        msg: "Modify Error"
    }), {
        status: 500
